	...
	scope {
		...
		try {
			for (va_copy(args_copy, args);
					(wanted_size = vsnprintf(new_string, array_length(new_string) + sizeof(char), fmt, args_copy)) > array_length(new_string);
...
char *string_new() {
	char *string = NULL;
	try {
		string = array_new(char, ARRAY_NULL_TERMINATED);
	};
	catch throw_proxy();
	return string;
};
...
char *string_new_set(const char *set) {
	return string_set_real(string_new(), set);
};
...
char **string_split(const char *string, const char *delimeter, int flags) {
	...
	char **out = array_new_real(sizeof(char *), ARRAY_NULL_TERMINATED);
